...
RFDataFile='C:/Users/Larmor/Desktop/501_anger_data/'
RFSIPMDataFile='C:/Users/Larmor/Desktop/501_anger_data/'
MuLog=open(RFDataFile+'logfile.txt','a')
...
#                        print("short payload received from %s = %d, expected = %d" % (self.socketID[0],totalin,ival[3]))
    ...
    def setsavelightcone(self,writefilename):
        ...
        self.myfile2=open(RFDataFile+'altlightconedata_gauss.txt','w+t')
...
#        pylab.show()
    ...
    def saverawpos(self,value,runnum):
        if value==1:
            self.saveraw=1
            cs=RFDataFile+ runnum
            self.rawfile=open(cs,'w+b')
            print('raw write file open'+runnum)
        else:
            if self.saveraw==1:
                self.rawfile.close()
...
class MyPSControl:
    ...
    def setcurrent(self,value):
        self.inst.write('SETI '+str(value))
        
    def getcurrent(self):
        cs=self.inst.query('RDGI?')
        return float(cs)
...
class MyFGControl:
    def __init__(self,ch):
        self.inst=rm.open_resource('USB0::2391::11271::MY57801733::0::INSTR')
        self.inst.timeout=200
...
#        self.on()
    ...
    def fre(self,value):
        self.inst.write(self.command+':FREQuency '+str(value))
    def phase(self,value):
        self.inst.write(self.command+':PHAse '+str(value))
    def on(self):
...
class MySiPMComm:
    def __init__(self,messagebox):
        ...
        messagebox.setText('attempting to connect to siPM at 1.10')
...
#bunch of tube varibles from the detector...local copies....
        ...
        self.whatplot=6  #default is psum, 1= a/d
...
#        self.threeDwidget.show()
        ...
        self.myscantimer=QtCore.QTimer()
        ...
        self.scanaquire.clicked.connect(self.StartScan)
...
#        self.myps1=MyPSControl(22)  ###power supply class
        self.scansteptime=self.steptime.value()
        self.numscansteps=self.scansteps.value()
        self.currentscanstep=0
        self.scanacquireflag=0
###############tbd###########
        ##set acutual current to value in start current or zero
        ##read acutrual current and update dialog value  self.ps1_actualcurrentvalue.setText()
#        self.myps1current=self.myps1.getcurrent()
#        cs='%.1f' % self.myps1current
#        self.ps1_actualcurrentvalue.setText(cs)
#        self.myps1startcurrent=self.ps1_startvalue.value()
#        self.myps1curstep=self.ps1_stepvalue.value()
        self.etm=0.0
...
#    def updateconfig_tree(self,configdic):
    ...
    def checkrb(self,b,value):
        if b.isChecked():
            ...
            if value==0:
                ...
                self.win2.setWindowTitle('TOF histo 1usec bins')
    ...
    def checksaverawposition(self,value):
        if self.saverawposition.isChecked():
            ...
            self.saveraw=0
            self.mystream1.saveraw(0,self.currentscanstep)
    ...
    def StreamAcquire(self):
        ...
        self.mytimer.start(2500)  #update every 2.5 seconds


    def StartScan(self):
        if self.scanacquireflag==1:
            return
        self.scanacquireflag=1
        self.scansteptime=self.steptime.value()
        print('scansteptime=%d' % self.scansteptime)
        self.numscansteps=self.scansteps.value()
        self.currentscanstep=0
        self.myscantimer.start(self.scansteptime*1000)
        print('scantimer started')
        self.SetPS1StartCur()        #set starting current here
        self.StreamAcquire()
        print('acquire started')

    def AcquireStop(self):
        self.scanacquireflag=0
        self.myscantimer.stop()
        self.AcquireStopProgram()
        
    def AcquireStopProgram(self):
        global zero_histos
        global global_acquireflag
        global twoD_histo
        zero_histos=0
        ...
        self.acquireflag=0
        self.mystream1.saverawpos(1,MURRrun)
...
##        writefile.close()
##        filename=RFDataFile+'adhistos.txt'
##        writefile=open(filename,'w+t')
...
##            writefile.write(mystr)
##        writefile.close()
        filename=RFSIPMDataFile+'sipm2d_'+MURRrun
        writefile=open(filename,'w+b')
        writefile.write(twoD_histo)
        writefile.close()
        filename=RFSIPMDataFile+'sipmtofhisto_'+MURRrun 
        writefile=open(filename,'w+b')
        writefile.write(tof_histo)
        writefile.close()
##        filename=RFDataFile+'psum3d.dat'
##        writefile=open(filename,'w+b')
...
##        writefile.close()
    ...
    def UpdatePlots(self):
        ...
        et=self.runningtime.elapsed()
        etm=et*0.001
        cs='acumm. time = %.1f sec....scan#=%d' % (etm,self.currentscanstep)
        self.stufflabel.setText(cs)
#whatplot is set in checkrb.  0 means plots 1-16 photosums 1 means plots a/d values.  (up to two tubes and a time.)
        if self.whatplot==6:
            ...
            self.plt2.plot(tof_histo_fine_xaxis,tof_histo_fine,clear=True)
            return
...
def nextrun():
    files = []
    for i in os.listdir(RFDataFile):
        if os.path.isfile(os.path.join(RFDataFile,i)) and 'CG4B' in i:

            files.append(re.search('CG4B(.+?).dat',i).group(1))

    return 'CG4B'+str( max(list(map(int, files)))+1)+'.dat'

def degauss(pol):

    print ("degaussing start")
    b2.setcurrent(0)

    while abs(b2.getcurrent())>0.1:
        print ("waiting to ramp down......"+str(b2.getcurrent()))
        time.sleep(10)

    b2.setcurrent(pol*15)
    print ("ramp to "+str(pol*15)+"A")
    time.sleep(150)

    b2.setcurrent(-pol*13)
    print ("ramp to "+str(-pol*13)+"A")
    time.sleep(280)
    
    b2.setcurrent(pol*11)
    print ("ramp to "+str(pol*11)+"A")
    time.sleep(240)  
    
    b2.setcurrent(-pol*9)
    print ("ramp to "+str(-pol*9)+"A")
    time.sleep(200)  
    
    b2.setcurrent(pol*7)
    print ("ramp to "+str(pol*7)+"A")
    time.sleep(160)  
    
    b2.setcurrent(-pol*5)
    print ("ramp to "+str(-pol*5)+"A")
    time.sleep(120)  
    
    b2.setcurrent(pol*4)
    print ("ramp to "+str(pol*4)+"A")
    time.sleep(90)  
    
    b2.setcurrent(-pol*3)
    print ("ramp to "+str(-pol*3)+"A")
    time.sleep(70)  
    
    b2.setcurrent(pol*2)
    print ("ramp to "+str(pol*2)+"A")
    time.sleep(50)  
    
    b2.setcurrent(-pol*1.5)
    print ("ramp to "+str(-pol*1.5)+"A")
    time.sleep(35)
    
    b2.setcurrent(pol*1)
    print ("ramp to "+str(pol*1)+"A")
    time.sleep(25)    
    
    b2.setcurrent(-pol*0.5)
    print ("ramp to "+str(-pol*0.5)+"A")
    time.sleep(15)
    
    b2.setcurrent(0)
    print ("ramp to 0A")
    time.sleep(5)
    print ("degaussing done")        

#--------------------------------------------------#
#Galil translation stage
def get_pulse(dist,pulse_per_mm=2500):
    return dist*pulse_per_mm

def get_dist(pulse,pulse_per_mm=2500):
    return pulse/pulse_per_mm

def tran_stage(dist):
    """Moves the sample translation stage by given dist in mm. Drives are always relative."""
    #current_pulse = galil_controller.GCommand('RPB')
    mm_per_sec = .6
    pulsenum = get_pulse(dist)
    pulse_string = 'PRB='+str(pulsenum)
    galil_controller.GCommand(pulse_string)  #set number of pulses to move
    galil_controller.GCommand('BGB')  #perform move

    print(f"Start position: {get_dist(int(galil_controller.GCommand('RPB')))}")
    print(f"Move time: {abs(dist/mm_per_sec)}")
    time.sleep(abs(dist/mm_per_sec))
    print(f"End position: {get_dist(int(galil_controller.GCommand('RPB')))}")
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    nu_pre=MyPSControl(11)
    WP1=MyPSControl(12)  #nu_post is not on lakeshore
    phase=MyPSControl(13)
    WP2=MyPSControl(14)
    ...
    galil_controller.GOpen('192.168.10.4') #old IP address
    #galil_controller.GOpen('10.112.116.1') #new IP address
    #print('Galil info:')
    ...
    #tran_stage(-2) #116 to go into and out of the beam

    #--------------------------------------------------#
    """run_description = ', test'
    scan_time = 10
    subscan_num = 1
    subscan_time = int(scan_time/subscan_num)
    for cur in [-5,5]:
        
        nu_pre.setcurrent(cur)
        phase.setcurrent(1)
        prism.setcurrent(1)
        time.sleep(10)
        
        for n in range(subscan_num):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=run_description+', subrun:'+str(n)

            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                +runnote+'_'\
                +'phase:'+str(phase.getcurrent())[:8]\
                +'_prism:'+str(prism.getcurrent())[:8]\
                +'_nu_pre:'+str(nu_pre.getcurrent())[:8]\
                +'_nu_post:'+str(5)[:8]+'\n')
            
            MuLog.flush()
            window.StreamAcquire()
            time.sleep(subscan_time)
            window.AcquireStop()
            window.SaveData()
            time.sleep(10)"""

    #--------------------------------------------------#
    """run_description = ', masked sample OUT, '
    scan_time = 15*60
    subscan_num = 10
    subscan_time = int(scan_time/subscan_num)
    for cur in [5]:
    ...
    tran_stage(116)  #move sample in

    run_description = ', masked sample IN, '
    scan_time = 15*60
    subscan_num = 10
    subscan_time = int(scan_time/subscan_num)
    for cur in [5]:
    ...
    #--------------------------------------------------#
    #5/27 Overnight AER scan in precession mode, Indention at CG is wrong!
    """
    scan_time = 2*60
    
    for MWP in numpy.linspace(2.5,5,9):
        prism.setcurrent(MWP)
        time.sleep(3)

        for _ in [0,1]:
            if abs(get_dist(int(galil_controller.GCommand('RPB'))))>=200:
                tran_stage(116)  #move sample in
                run_description = ', 444 IN'
            else:
                tran_stage(-116)  #move sample out
                run_description = ', 444 OUT'
                
                for CG in numpy.linspace(1,1.03,9):
                    phase.setcurrent(CG)
                    time.sleep(1)
                    
                    for nup in [5,-5]:
                        nu_pre.setcurrent(nup)
                        time.sleep(11)

                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=run_description
    ...
    """for samp in ['in','out']:
        if samp=='in':
            tran_stage(5)  #move sample in
        if samp=='out':
            tran_stage(-5)  #move sample out

        print('scanning '+samp)
        time.sleep(5)"""

    #--------------------------------------------------#
    #5/28 Morning AER scan in precession mode

    """scan_time = 2*60
    
    for MWP in numpy.linspace(.25,3,9):
        prism.setcurrent(MWP)
        time.sleep(3)
        ...
        for _ in [0,1]:
            ...
            for CG in numpy.linspace(1,1.03,8):
                phase.setcurrent(CG)
                time.sleep(1)
    ...
    #--------------------------------------------------#
    #5/28 Overnight flipping mode scan

    """scan_time = 20*60
    
    for MWP in numpy.linspace(.5,8,15):
        prism.setcurrent(MWP)
        time.sleep(5)
        ...
        for nup in [5,-5]:
            ...
            MURRrun=nextrun()
            time.sleep(1)
            runnote='Overnight flipping mode FR check'
    ...
    #--------------------------------------------------#
    #5/29 mask on detector, sample shielding
    ...
    for nup in [5,-5]:
        ...
        for n in range(20):
            ...
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', sample sail IN, FR check'
            
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                        +runnote+'_subscan:'+str(n)\
                        +'_phase:'+str(phase.getcurrent())[:8]\
                        +'_prism:'+str(prism.getcurrent())[:8]\
                        +'_nu_pre:'+str(nu_pre.getcurrent())[:8]\
                        +'_nu_post:'+str(5)[:8]+'\n')
    ...
    #--------------------------------------------------#
    #5/29 fringe test after rewiring
    """scan_time = 60

    nu_post.setcurrent(1)  #now first prism
    phase.setcurrent(1)
    ...
    for nup in [-5,5]:
        ...
        for n in range(5):
            ...
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', fringe check MWP1, pinhole'
            ...
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                        ...
                        +'_prism:'+str(prism.getcurrent())[:8]\
                        +'_nu_pre:'+str(nu_pre.getcurrent())[:8]\
                        +'_nu_post:'+str(nu_post.getcurrent())[:8]+'\n')
    ...
    #--------------------------------------------------#
    #5/29 fringe test after rewiring, scan phase current
    """scan_time = 60
    ...
    time.sleep(30)
    nu_post.setcurrent(1)  #now first prism
    phase.setcurrent(1.03)
    time.sleep(2)
    phase.setcurrent(1)
    time.sleep(2)
        
    for CG in numpy.linspace(1,1.03,10):
        phase.setcurrent(CG)
        time.sleep(2)
        ...
        for n in range(5):
            ...
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', fringe check MWP1, pinhole, CG scan'
    ...
    #--------------------------------------------------#
    #5/29 first SEMSANS precession mode check
    """scan_time = 30

    for MWP in numpy.linspace(-6,-7,5):
        prism.setcurrent(MWP)

        MURRrun=nextrun()
        time.sleep(1)
        runnote=', SEMSANS moved camera up'
    ...
    #--------------------------------------------------#
    #5/29 Fringe check after moving the detector
    """scan_time = 120
        
    for CG in numpy.linspace(-1,-1.5,10):
        phase.setcurrent(CG)
        time.sleep(5)

        MURRrun=nextrun()
        time.sleep(1)
        runnote=', fringe check MWP2, detector moved, CG scan'
    ...
    #--------------------------------------------------#
    #5/29 Checking SEMSANS with closer detector

    """nu_post.setcurrent(-2)
    time.sleep(45)

    scan_time = 30
    
    for CG in numpy.linspace(-1,-1.05,10):
        phase.setcurrent(CG)
        time.sleep(5)

        MURRrun=nextrun()
        time.sleep(1)
        runnote=', SEMSANS again'
    ...
    #--------------------------------------------------#
    #5/30 Checking SEMSANS with closer detector

    """scan_time = 240
    
    for CG in numpy.linspace(.5,.54,13):
        phase.setcurrent(CG)
        time.sleep(5)

        MURRrun=nextrun()
        time.sleep(1)
        runnote=', Fankang fringes,'
    ...
    for cur1 in [.5,1,1.5]:
        ...
        for cur2 in [0,.5]:
            ...
            for nup in [5,-5]:
                ...
                MURRrun=nextrun()
                time.sleep(1)
                runnote=', Fankang fringes pols,'
                
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                            +runnote\
                            +'_phase:'+str(phase.getcurrent())[:8]\
                            +'_WP1:'+str(WP1.getcurrent())[:8]\
                            +'_nu_pre:'+str(nu_pre.getcurrent())[:8]\
                            +'_WP2:'+str(WP2.getcurrent())[:8]+'\n')
    ...
    scan_time = 30 #real run

    nu_pre.setcurrent(3)
    phase.setcurrent(-1)
    WP1.setcurrent(1.1)
    WP2.setcurrent(2)
    time.sleep(15)  #timeout error when waiting 30 seconds???
    ...
    for nup in [3,-3]:
        ...
        for n in range(5):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', SEMSANS neg phase,'
            
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                        +runnote+'_subrun:'+str(n)\
                        +'_phase:'+str(phase.getcurrent())[:8]\
                        +'_WP1:'+str(WP1.getcurrent())[:8]\
                        +'_nu_pre:'+str(nu_pre.getcurrent())[:8]\
                        +'_WP2:'+str(WP2.getcurrent())[:8]+'\n')
    ...
    for pc in numpy.arange(.9,1.41,.05):
        WP1.setcurrent(pc)
        for nup in [3,-3]:
            nu_pre.setcurrent(nup)
            time.sleep(15)
            ...
            for n in range(10):
                MURRrun=nextrun()
                time.sleep(1)
                runnote=', Overnight SEMSANS tune,'
    ...
    #--------------------------------------------------#
    # 5/31 Morning scan, rotated s-bender
    ...
    for nup in [3,-3]:
        ...
        for n in range(5):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', SEMSANS with rotated s-benderm detector,'
    ...
    #--------------------------------------------------#
    # 5/31 Lunch scan, flipping mode regular image
    ...
    for nup in [4,-4]:
        ...
        for n in range(20):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', lunch scan FM 2 mm slit pattern,'
            ...
            time.sleep(1)
            window.SaveData()
            time.sleep(scan_time/2)  #time for saving data"""

    #--------------------------------------------------#
    # 5/31 Overnight AER scan

    """scan_time = 15 #real run

    nu_pre.setcurrent(-4)
    time.sleep(10)  #timeout error when waiting 30 seconds???

    for mwp_curs in numpy.linspace(.5,4.5,10):
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)

        for _ in [0,1]:
            if abs(get_dist(int(galil_controller.GCommand('RPB'))))>=200:
                tran_stage(116)  #move sample in
                run_description = ' 2mm IN,'
            else:
                tran_stage(-116)  #move sample out
                run_description = ' 2mm OUT,'

            for cg_curs in numpy.linspace(.5,.53,9):
                phase.setcurrent(cg_curs)
                time.sleep(1)
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER 2mm slit,'+run_description
                        ...
                        time.sleep(1)
                        window.SaveData()
                        time.sleep(5)  #time for saving data"""

    #--------------------------------------------------#
    # 6/1 FR flipping mode check, reserved phase polarity

    """scan_time = 20 #real run
    ...
    for nup in [-4,4]:
        ...
        for n in range(5):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', FR in FM (phase reversed),'
    ...
    for mwp_curs in numpy.linspace(2,4.5,5):
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
            ...
            for cg_curs in numpy.linspace(-.5,-.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER 2mm slit (phase rev.),'+run_description
    ...
    #--------------------------------------------------#
    # 6/2 Afternoon flipping mode check
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', Sample IN FM FR check, new s-bender mount,'
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', EMPTY FM FR check, new s-bender mount,'
    ...
    #--------------------------------------------------#
    # 6/2 Afternoon double slit (2-2-2) flipping mode check
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', .25 in graphite test FM, sample IN,'
    ...
    for mwp_curs in [1.5,1.8,2.1,4,8.15]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER, .25in graph,'+run_description
    ...
    #--------------------------------------------------#
    # 6/3 Overnight Graphite empty beam scan
    """scan_time = 20

    nu_pre.setcurrent(-4)
    time.sleep(10)

    for mwp_curs in numpy.linspace(3,6,15):
        #AER currents: [1.53,3.60,4.6,6.12,7.65]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
        ...
        for cg_curs in numpy.linspace(.5,.53,9):
            ...
            for nup in [-4,4]:
                ...
                for n in range(10):
                    MURRrun=nextrun()
                    time.sleep(1)
                    runnote=', Overnight EMPTY AER, .25in graph,'
    ...
    #--------------------------------------------------#
    # 6/4 FM FR check after degauss, two slit geometry
    # also did this with the sample in 
    """scan_time = 20
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', FM FR check, 2 slits, 222 IN,'
            ...
            time.sleep(5)  #time for saving data"""
    #--------------------------------------------------#
    # 6/4 Precession mode check, two slit geometry
    """scan_time = 20
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', Precession check, 2 slits, 222 IN,'
    ...
    #--------------------------------------------------#
    # 6/4 Overnight AER scan, double slit geometry
    ...
    for mwp_curs in [1,2,2.8,3,3.2,4,5,6]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER 2mm, double slit geometry,'+run_description
    ...
    #--------------------------------------------------#
    # 6/4 FR FM check, rotated v-cavity
    ...
    for nup in [-4,4]:
        ...
        for n in range(5):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', FR in FM, rotated vcavity,'
    ...
    #--------------------------------------------------#
    # 6/4 FR Precession mode FR check, rotated v-cavity
    ...
    for nup in [-4,4]:
        ...
        for n in range(5):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', FR in PreM, rotated vcavity, sample IN'
    ...
    #--------------------------------------------------#
    # 6/4 Overnight Rotated v-cavity AER scan
    """scan_time = 20 #real run

    nu_pre.setcurrent(-4)
    phase.setcurrent(.53)  #field training
    time.sleep(1)
    phase.setcurrent(.5)
    time.sleep(10)  #timeout error when waiting 30 seconds???

    for mwp_curs in [1,2,2.7,3,3.3,4,5,6]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER, rot vcav,'+run_description
    ...
    #--------------------------------------------------#
    # 6/6 Flipping mode/Precession mode check, back to single slit
    """scan_time = 10
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', PreM FR check, 19mm slit, 22 IN,'
    ...
    #--------------------------------------------------#
    # 6/6 Overnight Rotated v-cavity AER scan
    """scan_time = 10
    ...
    for mwp_curs in [1,1.5,2,2.5,3,4,5,6,7]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
        time.sleep(6)
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER 19mm slit,'+run_description
    ...
    #--------------------------------------------------#
    # 6/7 Continuation of last night's AER scan
    """scan_time = 10
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Day AER 19mm slit,'+run_description
    ...
    #--------------------------------------------------#
    # 6/7 Precession mode check with 3mm slit
    """scan_time = 20
    ...
    for nup in [-4,4]:
        ...
        for n in range(10):
            MURRrun=nextrun()
            time.sleep(1)
            runnote=', PreM FR check, 3mm slit, 22 OUT,'
    ...
    #--------------------------------------------------#
    # 6/7 Overnight 3mm AER scan, continuation for overnight 6/8
    scan_time = 25
    run_description = ' 2mm OUT,'

    for cg_curs in [0.5225,0.52625,0.53]:
        phase.setcurrent(cg_curs)
        time.sleep(1)
        ...
        for nup in [-4,4]:
            ...
            for n in range(10):
                MURRrun=nextrun()
                time.sleep(1)
                runnote=', Overnight AER 3mm slit, finish 7 after pause,'+run_description
    ...
    time.sleep(5)  #continuing MWP current scan

    for mwp_curs in [8,9,10]:
        WP1.setcurrent(mwp_curs)
        WP2.setcurrent(mwp_curs)
            ...
            for cg_curs in numpy.linspace(.5,.53,9):
                ...
                for nup in [-4,4]:
                    ...
                    for n in range(10):
                        MURRrun=nextrun()
                        time.sleep(1)
                        runnote=', Overnight AER 3mm slit,'+run_description
