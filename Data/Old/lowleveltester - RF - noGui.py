...
RFDataFile='C:/Users/Larmor/Dropbox/AngerCamera/Wide_Angle_NSE_2022/'
RFSIPMDataFile='C:/Users/Larmor/Dropbox/AngerCamera/Wide_Angle_NSE_2022_SIPM/'
...
class MyPSControl:
    def __init__(self,port):
        self.inst=rm.open_resource('GPIB1::'+str(port)+'::INSTR')
        self.inst.timeout=200
#        self.actualcurrent=0.0
#        self.currentsetpoint=0.0

    def getcurrent(self):
        #cs=self.inst.query('SETI?')
        cs=self.inst.query('RDGI?') #changed this to make getcurrent to work
        return float(cs)
    
    def setcurrent(self,value):
        self.inst.write('SETI '+str(value))
        #while(abs(self.getcurrent()-value)>0.05): #Fankang changed these two lines, timeout occured here
        #time.sleep(2)
...
#    def updateconfig_tree(self,configdic):
    ...
    def StreamAcquire(self):
        ...
        print ("Run Started")
#        self.mystream1.setsavelightcone(RFDataFile+'lightconedata_gauss.txt')
...
#    window.show()
    ...
    """#Wide-angle graphite sample scans
    n1cur = 5 #nut_pre
    n1.setcurrent(n1cur)
    n2cur = 5
    n2.setcurrent(n2cur)
    ...
    for B2 in [-5]:
        b2cur = B2   #NOT scanning chevron cf coil
        b2.setcurrent(b2cur)
        time.sleep(10)

        for B1 in numpy.arange(1,1.2,.005):    #scanning MWPs
            b1cur = B1
            b1.setcurrent(b1cur)
            time.sleep(2)
            
            for n in range(10): #number of subscans
                b1rcur = b1.getcurrent() #gets actual current from the lakeshore powersupply for log book
                b2rcur = b2.getcurrent()
                
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', graphite WA, 800 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(800) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    #Wide-angle overnight precession mode scan
    """n1cur = 5 #nut_pre
    n1.setcurrent(n1cur)
    n2cur = 5
    n2.setcurrent(n2cur)
    ...
    for B2 in [-5]:
        ...
        for B1 in numpy.arange(1,1.3,.005):    #scanning MWPs
            b1cur = B1
            b1.setcurrent(b1cur)
            time.sleep(2)
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', overnight pre. WA, 150 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(150) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    for B2 in [-5]:
        ...
        for B1 in numpy.arange(1,1.1,.11):    #scanning rectangular guide field
            b1cur = B1
            b1.setcurrent(b1cur)# timeout occured here
            time.sleep(2)
            
            for n in range(5): #number of subscans
                b1rcur = B1 #gets actual current from the lakeshore powersupply for log book
                b2rcur = B2
                
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', FM wide angle, 60 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(60) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    n1cur = 5 #nut_pre
    ...
    for B2 in [-15]:
        b2cur = B2   #NOT scanning correction coil
        b2.setcurrent(b2cur)
        time.sleep(10)
        
        for B1 in numpy.arange(-1,-1.15,-.01):    #scanning rectangular guide field
            b1cur = B1
            b1.setcurrent(b1cur)# timeout occured here
            time.sleep(2)
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', FM Correction coil, 40 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(40) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file"""
    ...
    for B2 in [-20]:
        ...
        for B1 in numpy.arange(-1,-1.15,-.01):    #scanning rectangular guide field
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', Correction coil after DG, 180 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(180) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file

    """
    """n1cur = 0 #turn nutators off for degaussing
    n1.setcurrent(n1cur)
    n2cur = 0
    n2.setcurrent(n2cur)
    print('Degauss b1 start')
    for dg in range(2,0,-1): #fine degauss, step of -1 in magnitude
        b1.setcurrent(dg)
        time.sleep(dg*2*10 + 10)
        print('b1 current:', b1.getcurrent())
        b1.setcurrent(-dg + 1)
        time.sleep((dg - 0.5)*2*10 + 10)
        print('b1 current:', b1.getcurrent())
    b1.setcurrent(0)
    time.sleep(5)
    print('Degauss b1 end')


    print('Degauss b2 start')
    for dg in range(20,0,-2): #fine degauss, step of -1 in magnitude
        b2.setcurrent(dg)
        time.sleep(dg*2*10 + 10)
        print('b2 current:', b2.getcurrent())
        b2.setcurrent(-dg + 1)
        time.sleep((dg - 0.5)*2*10 + 10)
        print('b2 current:', b2.getcurrent())
    b2.setcurrent(0)
    time.sleep(5)
    print('Degauss b2 end')

    print('Wiggle start')
    b2.setcurrent(18)
    time.sleep(190)
    for wg in [12,17,13,16,14,15.5,15]: #fine degauss, step of -1 in magnitude
        b2.setcurrent(wg)
        time.sleep(70)
        print('b2 current:', b2.getcurrent())
    print('Wiggle  end')
    
    n1cur = 5 #turn nutators back on after degauss
    n1.setcurrent(n1cur)
    n2cur = 5
    n2.setcurrent(n2cur)

    ##b2cur = 0   #NOT scanning correction coil
    ##b2.setcurrent(b2cur)
    ##b1cur = 0   #NOT scanning correction coil
    ##b1.setcurrent(b1cur)
    ...
    for B2 in [15]:
        ...
        for B1 in numpy.arange(-1,-1.15,-.01):    #scanning rectangular guide field
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', Correction coil after DG and Wiggle, 180 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
    ...
    n2.setcurrent(n2cur)
    print('Degauss b2 start')
    for dg in range(15,0,-2): #fine degauss, step of -1 in magnitude
        b2.setcurrent(dg)
        time.sleep(dg*2*10 + 10)
        print('b2 current:', b2.getcurrent())
        b2.setcurrent(-dg + 1)
    ...
    for B2 in [-15]:
        ...
        for B1 in numpy.arange(-1,-1.15,-.01):    #scanning rectangular guide field
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', Correction coil after DG and Wiggle, 140 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(140) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    for B2 in [15]:
        ...
        for B1 in numpy.arange(-1,-1.15,-.01):    #scanning rectangular guide field
            ...
            for n in range(10): #number of subscans
                ...
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', Correction coil after DG, 100 sec, 10Wx10Hmm slit, Real B1=' + str(b1rcur) + ' B2=' + str(b2rcur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(100) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    for B2 in [-15]:
        ...
        for B1 in numpy.arange(-1.14,-1.2,-.01):    #scanning rectangular guide field
            b1cur = B1
            b1.setcurrent(b1cur)
            time.sleep(2)
    ...
    for NUS in [1]:
        ...
        for n in range(3): #number of subscans
            MURRrun=nextrun()
            time.sleep(0.5)
            runnote=', MIEZE (Amp1=.14,Amp2=.16), 600 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                     ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                +runnote+'_'\
                +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
            MuLog.flush()
            window.StreamAcquire()
            time.sleep(600) #count time for subscan (ss)
            window.AcquireStop()
            window.SaveData()
            time.sleep(5) #needed to write to file
    ...
    for B1 in numpy.arange(8.49,8.69,.01):
        b1cur = B1
        b1.setcurrent(b1cur)

        for n in range(1): #number of subscans
            MURRrun=nextrun()
            time.sleep(0.5)
            runnote=', scan of B1 (MIEZE circuit, 540.4 kHz, .13 amp), 30 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                     ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                +runnote+'_'\
                +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
            MuLog.flush()
            window.StreamAcquire()
            time.sleep(30) #count time for subscan (ss)
            window.AcquireStop()
            window.SaveData()
            time.sleep(5) #needed to write to file
    b1.close()
    ...
    for N1 in [-5,5]:
        ...
        for B1 in numpy.arange(8.35,8.85,.02):
            b1cur = B1
            b1.setcurrent(b1cur)
            ...
            for n in range(5): #number of subscans
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', scan of B1 (MIEZE circuit, 540.4 kHz), 5 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(5) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    b1.close()
    ...
    #lunch scan in precession mode at 8 amps
    """n1=MyPSControl(11)
    n2=MyPSControl(12)
    b1=MyPSControl(13)
    b2=MyPSControl(14)
    ...
    n1cur = 5
    n1.setcurrent(n1cur) #NOT scanning nutators (in precession mode)
    n2cur = 5
    n2.setcurrent(n2cur)
    ...
    for sc in currents:
        ...
        for B2 in numpy.arange(-(sc + .1),-(sc - .1),.02):
            b2cur = B2
            b2.setcurrent(b2cur)
            ...
            for n in range(10): #number of subscans
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', Prec. NO RF (scan b2), 20 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(20) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    #overnight precession scan, NO RF
    """n1=MyPSControl(11)
    n2=MyPSControl(12)
    b1=MyPSControl(13)
    b2=MyPSControl(14)
    ...
    for B2 in numpy.arange(-19.55,-19.0,0.01):
        b2cur = B2
        b2.setcurrent(b2cur)
        ...
        for n in range(10): #number of subscans
            MURRrun=nextrun()
            time.sleep(0.5)
            runnote=', Prec. NO RF (scan b2), 40 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                     ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                +runnote+'_'\
                +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
    ...
    for B1 in numpy.arange(19.55,19.0,-0.01):
        b1cur = B1
        b1.setcurrent(b1cur)
        ...
        for n in range(10): #number of subscans
            MURRrun=nextrun()
            time.sleep(0.5)
            runnote=', Prec. NO RF (scan b1), 40 sec, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                     ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
            MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                +runnote+'_'\
                +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
    ...
    """#scan of RF2 (RF1 OFF) at 1.2 MHz
    n1=MyPSControl(11)
    n2=MyPSControl(12)
    b1=MyPSControl(13)
    b2=MyPSControl(14)
    ...
    b1cur = 19.55 #Expected static field resonance point of RF1
    b1.setcurrent(b1cur)
    n2cur = 5
    n2.setcurrent(n2cur) #NOT scanning downstream nutator

    count_time = [60, 15] #double the 'low count state, 'halve' the high count state (remember we expect one flip)

    for N1 in [-5,5]:
        n1cur = N1
        n1.setcurrent(n1cur)

        ss_time = count_time[int((N1 + 5)/10)] #60 for N1=-5, 15 for N1=5
        ...
        for B2 in numpy.arange(-19.55,-19.0,0.01):
            ...
            for n in range(10):
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', RF1 OFF RF2 ON, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                MuLog.flush()
                window.StreamAcquire()
                time.sleep(ss_time) #count time for subscan (ss)
                window.AcquireStop()
                window.SaveData()
                time.sleep(5) #needed to write to file
    ...
    """#scan of RF1 (RF2 OFF) at 1.2 MHz
    n1=MyPSControl(11)
    n2=MyPSControl(12)
    b1=MyPSControl(13)
    b2=MyPSControl(14)
    ...
    b2cur = -19.3 #Expected static field resonance point of RF2
    b2.setcurrent(b2cur)
    n2cur = 5
    n2.setcurrent(n2cur) #NOT scanning downstream nutator
    ...
    for N1 in [-5,5]:
        ...
        for B1 in numpy.arange(19.8,19.25,-0.025):
            b1cur = B1
            b1.setcurrent(b1cur)
            ...
            for n in range(10):
                MURRrun=nextrun()
                time.sleep(0.5)
                runnote=', RF1 ON RF2 OFF, 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                         ', N1=' + str(n1cur) + ', N2=' + str(n2cur) + ', subrun:' + str(n)
                MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                    +runnote+'_'\
                    +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
    ...
    """#Long B1 and B2 scans with no RF
    n1=MyPSControl(11)
    n2=MyPSControl(12)
    b1=MyPSControl(13)
    b2=MyPSControl(14)

    #b1cur, b2cur = [0,0] #need to be global for some reason...???, no typo "ell" instead of  "one"
    
    n2cur=5
    n2.setcurrent(n2cur) #NOT scanning downstream nutator

    count_time = [15, 60] #double the low count state, halve the high count state (remember NO flip)

    scanned_curs = [19] #current magnitudes to scan
    ...
    for sc in scanned_curs:
        ...
        for N1 in [-5,5]:
            ...
            ss_time = count_time[int((N1 + 5)/10)] #15 for N1=-5, 60 for N1=5
            
            for B2 in numpy.arange(-(sc + 0.5),-(sc - 0.5),.1):
                b2cur=B2
                b2.setcurrent(b2cur)
                ...
                for n in range(10):
                    MURRrun=nextrun()
                    time.sleep(0.5)
                    runnote=', RF OFF (scan B2), [15,60] sec for N1=[-5,5], 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                             ', N1=' + str(n1cur) + \
                             ', N2=' + str(n2cur) + ', subrun:' + str(n)
                    MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+MURRrun[:-4]\
                        +runnote+'_'\
                        +'B1:'+str(b1cur)[:8]+'B2:'+str(b2cur)[:8]+'N1:'+str(n1cur)[:8]+'N2:'+str(n2cur)[:8] + '\n')
                    MuLog.flush()
        ...
        for N1 in [-5,5]:
            ...
            for B1 in numpy.arange(sc + 0.5,sc - 0.5,-.1):
                b1cur=B1
                b1.setcurrent(b1cur)
                ...
                for n in range(10):
                    MURRrun=nextrun()
                    time.sleep(0.5)
                    runnote=', RF OFF (scan B1), [15,60] sec for N1=[-5,5], 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + \
                             ', N1=' + str(n1cur) + \
                             ', N2=' + str(n2cur) +', subrun:' + str(n)
                    MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                        +runnote+'_'\
    ...
    for n in [0,1,2]: #range(10):
        MURRrun=nextrun()
        time.sleep(0.5)
        runnote=', 10Wx10Hmm slit, B1=' + str(b1cur) + ', B2=' + str(b2cur) + ', nut_pre=' + str(n1cur) + \
                 ', nut_post=' + str(n2cur) +', subrun:' + str(n)
        MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
            +runnote+'_'\
            +'B1:'+str(b1cur)[:8]\
            +'B2:'+str(b2cur)[:8]\
    ...
    """for nu in [4]:
        for sgn in [1]:
            ...
            for para in numpy.arange(3,8,0.5): #numpy.arange(0,1,2):   #5.4 to 5.5 steps of 0.015
                for fine in numpy.linspace(para - 0.05,para + 0.05,10):
                    b2cur=-fine#16.9#16.85 #30.4
                    ...
                    for n in [0,1,2]: #range(10):
                        ...
                        time.sleep(0.5)
                        print ('fine:' + str(fine))
                        runnote=', 10Wx10Hmm slit, corr:-4.5A, prism at ' + str(fine) + ', subrun:'+str(n)
                        MuLog.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime())+'\t'+MURRrun[:-4]\
                            +runnote+'_'\
                            +'Corr_S:'+str(b1cur)[:8]\
                            +'_MWPs_:'+str(b2cur)[:8]\
