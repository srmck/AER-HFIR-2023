    ...
    def run(self):
        ...
        while connected==1:
            ...
            if len(data)==0 or self.killstream==1:
                ...
                break;
            if fileopen==1:
                myfile.write(data)  #get the rest of the data...
            ival=numpy.zeros(6)
...
#                        print("short payload received from %s = %d, expected = %d" % (self.socketID[0],totalin,ival[3]))
                ...
                if len(payload)==0 or self.killstream==1:
                    ...
                    #print(str(now))
                if fileopen==1:
                    myfile.write(payload)  #now deal with the payload.  
    ...
    def setsavelightcone(self,writefilename):
        ...
        self.myfile2=open(Data_path+'altlightconedata_gauss.txt','w+t')
    ...
    def handledata(self,payload,header_unpacked):
        ...
        if (header_unpacked[2] & 0xF000000)==0x2000000:  #this is normal mode
            for j in range(24,k,self.NORMAL_EVT_SZ):
                ...
                gcounts=gcounts+1
#                if (ixpos >= 0 and ixpos <= 511 and iypos >=0 and iypos <= 511):  #what is this?
...
class MyPSControl:
    ...
    def setcurrent(self,value):
        ...
        #while(abs(self.getcurrent()-value)>0.05): #Fankang changed these two lines, SPICE timeout occured here
            #time.sleep(2)
...
#    def updateconfig_tree(self,configdic):
    ...
    def StreamAcquire(self):
        ...
        print ("Run Started")
#        self.mystream1.setsavelightcone(Data_path+'lightconedata_gauss.txt')
    ...
    def AcquireStopProgram(self):
        ...
        self.acquireflag=0
        self.mystream1.saverawpos(1,Run_num)
...
##        writefile.close()
##        filename=Data_path+'adhistos.txt'
##        writefile=open(filename,'w+t')
...
##            writefile.write(mystr)
##        writefile.close()
        filename=SIPM_data_path+'sipm2d_'+Run_num
        writefile=open(filename,'w+b')
        writefile.write(twoD_histo)
        writefile.close()
        filename=SIPM_data_path+'sipmtofhisto_'+Run_num 
        writefile=open(filename,'w+b')
        writefile.write(tof_histo)
        writefile.close()
##        filename=Data_path+'psum3d.dat'
##        writefile=open(filename,'w+b')
...
def nextrun():
    ...
    for i in os.listdir(Data_path):
        if os.path.isfile(os.path.join(Data_path,i)) and 'CG4B' in i:
...
if __name__ == "__main__":
    ...
    nu_pre=MyPSControl(11)  #check these addresses
    ...
    nu_pre_I = 5  #current magnitude
    nu_pre.setcurrent(nu_pre_I)
    nu_post_I = 5  #always same sign
    nu_pre.setcurrent(nu_post_I)
    
    prism_currents = numpy.linspace(4,10,.3)
    phase_currents = numpy.linspace(.9,1.1,.02)
    nu_signs = [1,-1]
    subscan_number = 10
    scan_time = 300  #seconds
    subscan_time = int(scan_time/subscan_number)
    run_description = "Explain!"
    
    for I_MWP in prism_currents:
        MWP.setcurrent(I_MWP)
        time.sleep(2)
        print("***Prism current: "+str(I_MWP)+"***")
        
        for I_phase in phase_currents:
            CG.setcurrent(I_phase)
            time.sleep(2)
            print("**CG current: "+str(I_MWP)+"**")
            
                for nu_sign in nu_signs:
                    nu_pre_I.setcurrent(nu_pre_I*nu_sign)
                    time.sleep(10)
                    ...
                    for sample in ['in','out']:
                        ...
                        for n in range(subscan_number):             
                            Run_num=nextrun()
                            time.sleep(1)
                            runnote=run_description+', CG=' + str(round(CG.getcurrent(),5)) + ', MWP=' + str(round(MWP.getcurrent(),5)) + \
                            ', nu1=' + str(round(nu_pre.getcurrent(),5)) + ', nu2=' + str(round(nu_post.getcurrent(),5)) + ', subrun:' + str(n)
                            
                            Log_file.write(time.strftime('%a, %d %b %Y %H:%M:%S', time.gmtime()) + '\t'+Run_num[:-4] + runnote + '\n')
                        
                            Log_file.flush()
                            window.StreamAcquire()
                            time.sleep(subscan_time) #count time in seconds for each subscan
                            window.AcquireStop()
                            window.SaveData()
                            time.sleep(10) #needed to write to file, increase this time to prevent data loss?
                            print ('subscan_'+str(n)+' finished.')
